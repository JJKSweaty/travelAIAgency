    ...
    <div className="rounded-lg bg-ink p-5 text-paper shadow-subtle">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs text-paper/52">Estimated spend {formatMoney(budget.totalEstimated, currency)}</p>
        </div>
      ...
      <div className="mt-4 grid gap-2 text-sm">
        {categories.map((category) => (
          <div key={category.label} className="rounded-lg bg-white/8 px-3 py-2">
            ...
            <div className="mt-2 h-1.5 rounded-full bg-white/10">
              <div className="h-1.5 rounded-full bg-white/48" style={{ width: `${Math.min(100, Math.max(6, Math.round((category.value / Math.max(1, budget.totalEstimated)) * 100)))}%` }} />
            </div>
          </div>
        ))}
      </div>
      <div className="mt-4 rounded-lg bg-white/8 p-3">
        <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.13em] text-paper/58">
          <WalletCards size={14} aria-hidden />
          Budget moves
        </p>
        <div className="mt-3 grid gap-2">
          {recommendations.map((recommendation) => (
            <p key={recommendation} className="text-sm leading-5 text-paper/74">
              {recommendation}
            </p>
          ))}
        </div>
...
function BudgetActionContent({ action }: { action: BudgetAction }) {
  return (
    <>
      ...
      <span className="mt-1 block text-xs leading-5 text-ink/56">{action.description}</span>
    </>
  );
}
